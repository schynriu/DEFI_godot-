...
func _process(delta):
	...
	for body in bodies: # step throu chaque body qui est dans cette liste
		if not touche and body.name == "player": #les deux egale signifie verifier, sortir vrai ou faux
			get_node("../immigration/player/UI/MarginContainer/HPbar").cpt_vie -=5 #ceci permet de faire en sorte que la porte nous emmenera au lvl qu'on voudra, en le modifiant dans les nod  
			#get_node("../immigration/player").position = Vector2(151, 478)
			get_node("../immigration/player").position = get_node("../spawn").position
			touche = true