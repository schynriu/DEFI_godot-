...
func _ready():
	...
	button = get_node("../../player/player/UI/fenetre/Button")
	button.connect("pressed", self, "_on_Button_pressed")
	dialogue = get_node("../../player/player/UI/fenetre/Label")
	pass
...
func _process(delta):
	if activer :
		...
		match discution:
			1:
				dialogue.text = "In the beginning... The angel Lucifer was cast out of Heaven and condemned to rule Hell for all eternity... Until..."
			
			2:
				get_node("../../player/player/UI/fenetre").hide()
				dialogue.text = ""
				activer = false
		pass
	...
	for body in bodies: # step throu chaque body qui est dans cette liste
		if body.name  == "player": #les deux egale signifie verifier, sortir vrai ou faux
			...
			if Input.is_action_pressed("ui_action"):
				get_node("affBtn").hide()
				get_node("../../player/player/UI/fenetre").show()
				activer = true
				discution = 1
			pass 
	pass
