...
func _physics_process(delta):
	...
	for body in bodies: # step throu chaque body qui est dans cette liste
		if body.name == "player": #les deux egale signifie verifier, sortir vrai ou faux
			get_tree().change_scene("res://UI/intro/Intro.tscn") #ceci permet de faire en sorte que la porte nous emmenera au lvl qu'on voudra, en le modifiant dans les nod  
